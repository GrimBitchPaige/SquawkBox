...
func _gui_input(event: InputEvent) -> void:
	#TODO: fix the popup position when floating
...
func nodes_to_json() -> void:
	...
	var tmp_condition_dict : Dictionary = {}
	for node in conditions_nodes:
		var tmp_dict : Dictionary = node.export_conditions()
		print(tmp_dict)
		if tmp_condition_dict.has(tmp_dict['node_id']):
			print('squawkdock.nodes_to_json hit tmp_conidition_dict has node_ID')
			var another_dict : Dictionary = tmp_dict.duplicate(true)
			another_dict.erase('node_id')
			another_dict.erase('reply_num')
			tmp_condition_dict[tmp_dict['node_id']].merge({tmp_dict['reply_num']:another_dict})
		else:
			print('squawkdock.nodes_to_json hit tmp_conidition_dict missing node_ID')
			var another_dict : Dictionary = tmp_dict.duplicate(true)
			another_dict.erase('node_id')
			another_dict.erase('reply_num')
			tmp_condition_dict[tmp_dict['node_id']] = {tmp_dict['reply_num']: another_dict}
		reply_conditions_dict.merge(tmp_condition_dict)
	
	export_dict["data"] = data_dict
	export_dict["connections"] = connections_dict
	export_dict["conditions"] = reply_conditions_dict
...
func json_to_nodes() -> void:
	...
	for key in tmp_dict["conditions"]:
		var nodes_handled : Dictionary = {}
		if not nodes_handled.has(key):
			for reply_number in tmp_dict["conditions"][key]:
				var condition_n : Node = conditions_node.instantiate()
				condition_n.visible = false
				condition_n.call_deferred("set_label_text",tmp_dict["data"][key]["Character"], key, int(reply_number))
				condition_n.call_deferred("import_conditions", key, tmp_dict["conditions"][key][reply_number])
				conditions_nodes.append(condition_n)
				add_child(condition_n)
			nodes_handled[key] = 1
		else:
			push_error("Error: an error occurred while creating condition node " + key + '\nnode id found in conditions more than once, this should not happen\n')
